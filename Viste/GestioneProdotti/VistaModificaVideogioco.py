...
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QLabel, QHBoxLayout, QSizePolicy, QLineEdit, \
    QComboBox, QSpacerItem, QPushButton, QMessageBox

from Classi.Videogioco import Videogioco
from Gestori.GestoreProdotto import GestoreProdotto

class VistaModificaVideogioco(QWidget):
    ...
    def __init__(self, videogioco):
        try:
            ...
            label_titolo = QLabel("Modifica videogioco")
            ...
            self.line_nome.setText(videogioco.nome)
            ...
            self.line_anno.setText(str(videogioco.anno))
            ...
            self.combo_genere.addItems(["Azione", "Avventura", "RPG (Gioco di ruolo)", "Sparatutto",
                                                "Sportivo", "Simulazione", "Strategia", "Puzzle", "Musicale", "Horror"])
            self.combo_genere.setCurrentText(videogioco.genereVideogioco)
            ...
            self.line_prezzo_acquisto.setText(str(videogioco.costoAcquisto))
            ...
            self.line_prezzo_noleggio.setText(str(videogioco.costoNoleggio))
            ...
            self.line_unita.setText(str(videogioco.unita))
            ...
            label_regista = QLabel("Piattaforma:")
            ...
            self.combo_piattaforma.setCurrentText(videogioco.piattaforma)
            layout9.addWidget(self.combo_piattaforma)
            layout9.setAlignment(self.combo_piattaforma, Qt.AlignRight)
    ...
    def on_click_annulla(self):
        ...
        self.vista_gestione_videogioco = VistaGestioneVideogioco(self.videogioco)
    ...
    def on_click_conferma(self):
        try:
            ...
            videogioco_modificato = Videogioco(self.videogioco.id, nome, anno, prezzo_acquisto, prezzo_noleggio, disponibile, unita, "Videogioco", genere, piattaforma)

            gestore_prodotto = GestoreProdotto()
            gestore_prodotto.modifica_prodotto(videogioco_modificato)

            QMessageBox.information(self, 'Successo', 'Videogioco modificato con successo', QMessageBox.Ok)

            from Viste.GestioneProdotti.VistaGestioneVideogioco import VistaGestioneVideogioco
            self.vista_gestione_videogioco = VistaGestioneVideogioco(videogioco_modificato)
            self.vista_gestione_videogioco.show()
            self.close()
        except Exception as e:
